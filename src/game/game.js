...
import { strings } from "../data/globalData.js";
...
export async function listTasksPrompt(path = strings.tasksJsonPath) {
    ...
    for (let task = 0; task < tasks.length; task++) {
        let taskFromFile = Task.fromJson(tasks[task]);
        console.log(`${task + 1} - ${taskFromFile.toString()}`);
    }
...
export async function addTaskPrompt() {
    while (true) {
        ...
        const taskTitle = await rl.question("Task title: "); // Read task title input from user
        const taskBody  = await rl.question("Task body: ");
        const dueDate   = await rl.question("Due date: ");
        let taskObj = new Task(taskTitle, taskBody, dueDate);

        writeTaskToFile(strings.tasksJsonPath, taskObj.toJson()); // Save task to file
